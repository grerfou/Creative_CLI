...
Programme permettant de crée un template de programe processing (.pde)
...
int main() {
   if (mkdir("projet", 0777) == -1) {                                                //  Crée dossier "projet" 
    ...
    char *main_c_content = read_file("../template/processing_template.txt");       //  Lit le fichier 
    if (main_c_content == NULL) {
        return 1;                                                                     //  1 si erreur
    }

    if (write_file("projet.pde", main_c_content) != 0) {                                  //  Ecrit main.c 
    ...
    printf("projet.pde écrits à l'intérieur de src.\n");
